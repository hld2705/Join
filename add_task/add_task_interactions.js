...
/**
 * Sets active priority and updates visual state.
 *
...
/** Toggles the assign dropdown arrow icon. */
...
function clearAssignedInput() {
  ...
  clearAssignedIcon();
  document.getElementById('category-container').classList.remove('margin-top20');
}

/** Hides all assigned user check icons. */
...
/** Updates spacing based on assigned user selection state. */
function getAssignedSpace() {
  let hasAnySelected = document.querySelector('.Assigned-dropdown-username.bg-grey');
...
/**
 * Toggles check button state for assigned user.
 *
 * @param {HTMLElement} isInsideAssigned
 */
...
/** Toggles the category dropdown arrow icon. */
...
function closeAssignedInputOutclick(e) {
  ...
  if (
    e.target.id !== 'assign-input' &&
    e.target.id !== 'drop-down-svg-assign' &&
...
/** Resets the selected task category. */
...
/**
 * Toggles focus styling for the subtask input field.
 *
 * @param {Event} e
 */
...
/** Creates and returns a check button element for assigned users. */
